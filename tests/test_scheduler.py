...
class TestSendReminders:
    ...
    @patch('app.helpers.reminder_scheduler.get_engine')
    @patch('app.helpers.reminder_scheduler.send_event_reminder_email')
    def test_send_reminders_with_multiple_event_dates_same_event(
        self, 
        mock_send_email: Mock, 
        mock_get_engine: Mock, 
        test_session: Session,
        ...
        """Test that reminders work correctly when an event has multiple dates for tomorrow."""
        # Setup multiple event dates for the same event
        tomorrow = (datetime.datetime.now() + datetime.timedelta(days=1)).date()
        
        event_date1 = EventDate(
            event_id=sample_event_tomorrow.id or 0,
            day_date=tomorrow,
            start_time=datetime.time(9, 0),
            end_time=datetime.time(12, 0),
        )
        ...
        # Verify only one email is sent per registration, but with all dates
        mock_send_email.assert_called_once()
        call_args = mock_send_email.call_args[0]
        assert call_args[0].id == sample_user.id  # User
        assert call_args[1].id == sample_event_tomorrow.id  # Event
        assert isinstance(call_args[2], list)  # EventDates list
        assert len(call_args[2]) == 2  # Both event dates should be included